...
if(!function_exists('\ProcessWire\fetchOpenAIModels')) {
    function fetchOpenAIModels($configData): array
    {
        ...
        if (empty($data['data']) || !is_array($data['data'])) return [];
        ...
        foreach ($data['data'] as $item) {
            ...
            $id = $item['id'] ?? '';
            if (!$id || !str_starts_with($id, 'gpt-5')) continue;

            // exclude dated snapshots like gpt-5-mini-2025-02-10
            if (preg_match('/-\d{4}-\d{2}-\d{2}$/', $id)) continue;

            // exclude unwanted variants
            foreach ($excludeTerms as $term) {
                if (str_contains($id, $term)) continue 2;
            }

            $models[$id] = $id === 'gpt-5-mini'
                ? "$id (recommended)"
                : $id;
        }
...
if(!function_exists('\ProcessWire\refreshModels')) {
    function refreshModels(array $configData): array
    {
        ...
        if ($models) {
            ...
            wire()->warning(__('Could not fetch models from OpenAI. Keeping existing model list.'));
        }
        return $configData;
    }
}
...
if($inputPost) {
    //$updatedModelList = refreshModels($configData);
...
$config = [
            ...
            [
                ...
                'children' => [
                    ...
                    [
                        ...
                        'columnWidth' => 20,
                        'notes' => __('Number of blacklist strikes before being blocked'),
                        'value' => 3,
                    ],
                ],
            ],
    ...
    [
        ...
        'collapsed' => 1,
        'description' => __('Development-only tools for testing edge cases. These settings should remain OFF in production.'),
        'children' => [
            [
                'name' => 'debug_enabled',
                'type' => 'checkbox',
            ...
            [
                ...
                'notes' => __('Truncates the assistant response server-side, sanitises to plain text, and sets cutoff=true.'),
            ...
            [
                ...
                'notes' => __('For Characters: number of chars. For Words: number of words. For Awkward: base char count before stress text is appended.'),
                'columnWidth' => 33,
                'showIf' => 'debug_enabled=1, debug_force_cutoff=1',
                'value' => 200,
            ],
            [
                ...
                'notes' => __('Adds a server-side delay (superuser only) to test busy guards and UX states.'),
                'options' => [
                    '0' => __('None'),
                    '250' => __('250 ms'),
                    '1000' => __('1 s'),
