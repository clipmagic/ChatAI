...
trait HeadingsOnlyExtractor
{
    ...
    {
        $empty = ['h1' => '', 'h2' => [], 'h3' => []];
        if (!$page->id || !$page->isPublic()) return $empty;
        ...
        if ($html === '') { $cache->saveFor('chatai', $key, $empty, 1800); return $empty; }
    ...
    protected function renderPageAsGuest(Page $page, int $langId): string
    {
        ...
        $prevLang = ($languages && $prevUser && $prevUser->language) ? $prevUser->language : null;
        if ($languages && $langId) {
    ...
    protected function extractH123FromHtml(string $html): array
    {
        ...
        $h2 = [];
        foreach ($h2List as $n) { $t = $norm($n->textContent); if ($t !== '') $h2[] = $t; }
        $h3 = [];
        foreach ($h3List as $n) { $t = $norm($n->textContent); if ($t !== '') $h3[] = $t; }
